...
Provide step-by-step instructions on how a user can install the mod manually if they aren't using the Steam Workshop. 
...
* List prerequisites (e.g., specific versions of Node.js, TypeScript, .NET SDK, or game dependencies).
* Provide the terminal commands needed to clone, install dependencies, and compile the code (e.g., `npm install` and `npm run build`, or `dotnet build`).
...
> 
> **Forking Policy:** Ecosystem fragmentation hurts the community. If your fork primarily consists of bug fixes or feature additions that align with the core vision of this mod, I request that your changes be submitted as a Pull Request to this existing codebase rather than being published as a completely separate standalone release on Steam or GitHub. Let's work together to make the main branch as robust as possible.
...
## 📜 License
If this mod is a fork, explicitly state that it inherits the original mod's license and link to the original repository. 

If it is an original mod, include the following exact text:
> Released under the **MIT License**. 
> 
> **Attribution Requirement:** You are free to use and modify this code, provided that you credit me and link back to this project in your Git repository, your Steam Workshop page description, and the in-game mod info file.
...
When I provide you with the raw notes, code, or features for a new mod, respond ONLY with the fully formatted Markdown. Do not wrap the entire output in a single markdown code block if it prevents standard rendering, just provide the raw Markdown text.
